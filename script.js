...
formButton.addEventListener("submit", (e) => {
  ...
  //   console.log(
  //     "🚀 ~ file: script.js ~ line 19 ~ formButton.addEventListener ~ randomNumber",
  //     randomNumber
  //   );

  const container = document.querySelector(".container");
  const input = document.querySelector("#paragraph").value;
  let inputValue = parseInt(input);

  const textList = text.slice(0, inputValue);
  if (
    isNaN(inputValue) ||
    inputValue < 0 ||
    inputValue > 9 ||
    ...
    container.innerHTML = `<p class="result" >${text[randomNumber]}</p>`;
  } else {
    const paragraph = textList
      .map((text) => {
